...
export default function FeaturedCard({ imgUrl, title, href }) {
    return (
        <Link className={`relative aspect-[12/18] max-w-72 rounded-lg overflow-hidden border flex-1 group`} href={href}>
            ...
            <div className="absolute top-0 left-0 w-full h-full bg-shadow">
                <div className="absolute p-4 bottom-0 w-full text-neutral-50">
                    <div className="text-2xl text-center group-hover:text-primary">{title}</div>
                </div>
            </div>
        </Link>
    )
